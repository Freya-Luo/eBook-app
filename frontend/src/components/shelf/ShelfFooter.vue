...
export default {
  ...
  methods: {
    ...
    downloadBook(book) {
      ...
      return new Promise((resolve, reject) => {
        download(
          book,
          (book) => {
            // create-api: cannot update text, once the value has been passed, it cannot be modified => have to remove the component
            toast.remove()
            resolve(book)
          },
          reject,
          (progressEvent) => {
            ...
            // This is also why we using this.showText in the Toast.vue, this.text is ''
    ...
    clearCachedBook() {
      Promise.all(this.shelfSelected.map((book) => this.clearBook(book))).then(
        (books) => {
          books.map((book) => {
            book.cache = false
          })
    ...
    async setDownload() {
      this.onComplete()
      if (this.isDownload) {
        this.clearCachedBook()
      } else {
        /* downloadSelectedBook() is async but simpleToast() is sync.
        Using async/await label, it will treat the function as sync. */

...
export default {
  ...
  methods: {
    show() {
      this.ifNewGroup = this.showNewGroupTitle
    ...
    moveToGroup(group) {
      // rule out the selected books and update the ShelfList cause selected books will be merged
      this.setShelfList(
    ...
    createNewGroup() {
      ...
      /* case 1: just change group name. */
      if (this.showNewGroupTitle) {
        this.shelfCategory.title = this.newGroupName
        this.onComplete()
      } else {
        /* case 2: add a new group. */
        const group = {
          // the last element is "add" DOM, which id is -1
